...
def test_store_and_sanitize_moves_secrets_to_permission_restricted_file(tmp_path):
    config = {
        "accounts": {
            "kktix_password": "secret-password",  # pragma: allowlist secret
            "kktix_account": "user@example.com",
        },
        "advanced": {
            "telegram_bot_token": "123456:ABCdefghijklmnopqrstuvwxyz_123",  # pragma: allowlist secret
        },
    }
    ...
    assert stored["accounts.kktix_password"] == "secret-password"  # pragma: allowlist secret
...
def test_hydrate_restores_secrets_without_changing_preferences(tmp_path):
    ...
    store.set_many({
        "accounts.kktix_password": "secret-password",  # pragma: allowlist secret
        "advanced.telegram_bot_token": "123456:ABCdefghijklmnopqrstuvwxyz_123",  # pragma: allowlist secret
    })
